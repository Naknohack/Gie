...
// Lưu trạng thái nhạc đang phát riêng cho từng server (guildId -> { connection, player, process })
...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.GuildVoiceStates // Bắt buộc bật để lệnh !mp phát nhạc trong kênh thoại hoạt động được
  ]
});
  ...
  try {
    ...
    if (msg.mentions.has(client.user)) {
      ...
      try {
        ...
        // Nếu người đang hỏi là chủ sở hữu bot (OWNER_IDS[0]), yêu cầu Gemini luôn xưng hô gọi họ là "sếp".
        const bossInstruction = msg.author.id === "1020868400672686080"
          ? ` Người đang trò chuyện với bạn ngay bây giờ chính là chủ nhân/chủ sở hữu của bạn. Hãy luôn gọi người này là "sếp" một cách tôn trọng và thân thiện trong suốt cuộc trò chuyện (ví dụ: "Dạ sếp,...", "Sếp cần gì ạ?"), thay vì gọi bằng tên hoặc các đại từ khác.`
          : "";
        ...
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-lite-latest:generateContent?key=${GEMINI_API_KEY}`,
          {
            ...
            body: JSON.stringify({
              system_instruction: {
                parts: [
                  { text: `Bạn tên là "Noctis Core", một trợ lý AI trên Discord được phát triển bởi Naknohack. Nếu ai hỏi bạn là ai, tên gì, hoặc ai tạo ra bạn, hãy trả lời rõ bạn là Noctis Core, do Naknohack phát triển. Hãy trả lời ngắn gọn, thân thiện. Luôn trả lời bằng đúng ngôn ngữ mà người dùng đang sử dụng để hỏi (ví dụ: hỏi tiếng Anh thì trả lời tiếng Anh, hỏi tiếng Việt thì trả lời tiếng Việt, hỏi tiếng Trung, Nhật, Hàn... thì trả lời bằng ngôn ngữ đó tương ứng), không mặc định chỉ dùng một ngôn ngữ cố định. Thời gian thực tế hiện tại là: ${currentDateTimeVN} (giờ Việt Nam, GMT+7). Luôn dùng mốc thời gian này làm chuẩn khi trả lời các câu hỏi liên quan đến ngày giờ, đừng dùng ngày tháng trong dữ liệu huấn luyện cũ của bạn.${bossInstruction}` }
                ]
              },
              contents
            })
...
client.on("messageCreate", async msg => {
  try {
    if (msg.author.bot || !msg.guild) return;
    if (!msg.content.startsWith("!mp")) return;

    const args = msg.content.trim().split(/\s+/);

    // ---- !mp stop : dừng nhạc + rời kênh thoại ----
    if (args[1] === "stop") {
      const data = musicPlayers.get(msg.guild.id);
      if (!data) {
        return msg.reply("<a:1000079263:1530505382911283380> Hiện không có nhạc nào đang phát.");
      }
      try { data.process.kill("SIGKILL"); } catch {}
      try { data.player.stop(true); } catch {}
      try { data.connection.destroy(); } catch {}
      musicPlayers.delete(msg.guild.id);
      return msg.reply("<a:1000079259:1530505379287404544> Đã dừng phát nhạc và rời kênh thoại.");
    }

    // ---- !mp <link> : phát nhạc ----
    const url = args[1];
    if (!url || !/^https?:\/\//i.test(url)) {
      return msg.reply("Cú pháp: `!mp <link YouTube>`\nDừng nhạc: `!mp stop`");
    }

    const voiceChannel = msg.member.voice.channel;
    if (!voiceChannel) {
      return msg.reply("<a:1000079263:1530505382911283380> Bạn cần vào một kênh thoại (voice channel) trước đã.");
    }

    const me = msg.guild.members.me;
    const vPerms = voiceChannel.permissionsFor(me);
    if (!vPerms.has(PermissionsBitField.Flags.Connect) || !vPerms.has(PermissionsBitField.Flags.Speak)) {
      return msg.reply("<a:1000079263:1530505382911283380> Bot đang thiếu quyền **Kết Nối (Connect)** hoặc **Nói (Speak)** ở kênh thoại này. Vào Server Settings > Roles, kiểm tra role của bot.");
    }

    // Nếu đang phát ở server này rồi thì dừng bài cũ trước khi phát bài mới
    const old = musicPlayers.get(msg.guild.id);
    if (old) {
    ...
    const loadingMsg = await msg.reply("⏳ Đang tải và kết nối...");
    ...
    try {
      ...
      // Dùng yt-dlp để lấy luồng audio trực tiếp (stream), không tải file về ổ đĩa
      ...
      ytdlp.stderr.on("data", () => {}); // im lặng log lỗi phụ của yt-dlp
      ...
      musicPlayers.set(msg.guild.id, { connection, player, process: ytdlp });

      player.on(AudioPlayerStatus.Idle, () => {
        // Bài hát phát xong tự nhiên -> dọn dẹp
        try { ytdlp.kill("SIGKILL"); } catch {}
        musicPlayers.delete(msg.guild.id);
      });

      player.on("error", err => {
        console.error("Lỗi phát nhạc (!mp):", err);
        msg.channel.send("<a:1000079263:1530505382911283380> Có lỗi khi phát nhạc, có thể link không hợp lệ hoặc bị chặn.");
        try { ytdlp.kill("SIGKILL"); } catch {}
        musicPlayers.delete(msg.guild.id);
      });
      ...
      await loadingMsg.edit(`▶️ Đang phát nhạc trong **${voiceChannel.name}**\n${url}`);
    } catch (err) {
      console.error("Lỗi lệnh !mp:", err);
      musicPlayers.delete(msg.guild.id);
      await loadingMsg.edit("<a:1000079263:1530505382911283380> Không thể phát nhạc. Kiểm tra lại link, hoặc kiểm tra yt-dlp/ffmpeg đã cài đúng chưa.");
    }
  } catch (err) {
    console.error("Lỗi xử lý lệnh !mp:", err);
  }
});
